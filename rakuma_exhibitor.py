...
def process_products():
    # 1. ラクマに出品済みの品番を読み込む
    rakuma_hinban_set = set()
    try:
        with open("products_rakuma.csv", "r", encoding="utf-8-sig", errors="replace") as f:
            reader = csv.reader(f)
            header = next(reader)
            hinban_idx = header.index('品番')
            for row in reader:
                if len(row) > hinban_idx and row[hinban_idx]:
                    rakuma_hinban_set.add(row[hinban_idx])
        log(f"📚 ラクマ商品 {len(rakuma_hinban_set)} 件の品番を読み込みました。")
    except Exception as e:
        log(f"⚠️ products_rakuma.csv 読み込みエラー: {e}")

    # 2. メルカリCSVから、ステータスが2で、かつラクマに存在しない商品のIDを抽出
        ...
        with open("products_mercari.csv", "r", encoding="utf-8-sig", errors="replace") as f:
            ...
            for row in reader:
                if len(row) > max(hinban_idx, status_idx, url_idx):
                    ...
                    if status == '2' and hinban not in rakuma_hinban_set:
                        product_id = url.replace('https://jp.mercari.com/shops/product/', '')
                        if product_id:
                            target_product_ids.add(product_id)
        log(f"🔍 抽出条件: {len(target_product_ids)} 件のメルカリ商品を対象とします (ステータス=2, ラクマに無し)。")
    except Exception as e:
    ...
    with sync_playwright() as p:
        ...
        for i, row in enumerate(products):
            ...
            try:
                ...
                # --- サイズ ---（CSV にサイズがあれば反映。モーダルで選択する処理を追加）
                try:
                    # CSV にサイズ列があれば取得。列名は「サイズ」を想定（なければデフォルト値を使用）
                    size_label = None
                    if indices.get('サイズ') is not None:
                        try:
                            size_label = row[indices['サイズ']].strip()
                            if not size_label:
                                size_label = None
                        except Exception:
                            size_label = None
                    # デフォルトサイズ（CSV で値がない場合に使用）
                    default_size = "FREE / ONESIZE"
                    target_size_text = size_label or default_size
                    ...
                    # 優先: モーダル内の代表的な選択肢要素
                    candidates = [
                        f'section[role="dialog"] div.css-17rawrb:has-text("{target_size_text}")',
                        f'section[role="dialog"] div:has-text("{target_size_text}")',
                        f'div.css-17rawrb:has-text("{target_size_text}")',
                        f'div:has-text("{target_size_text}")',
                    ]
                    ...
                    # 3) フォールバック: 部分一致で探す（例: CSV が "FREE" のみなど）
                    if not picked and size_label:
                        try:
                            loc = page.locator(f'section[role="dialog"] div.css-17rawrb').filter(has_text=size_label)
                            if loc.count() > 0:
                                safe_click(loc.first)
                                picked = True
                        except Exception:
                    ...
                    if picked:
                        log(f"✅ サイズを選択しました: {target_size_text}")
                        ...
                        try:
                            ...
                            try:
                                ...
                                btn_text = ""
                            if target_size_text not in btn_text:
                                # 強制セット
                                page.evaluate("""(t) => {
